...
const VideoBar: React.FC<VideoBarProps> = ({
  ...
  const handleZoomChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const sliderValue = Number(e.target.value); // 0 - 100
    if (prevWidthPercentRef.current !== sliderValue) {
      prevWidthPercentRef.current = sliderValue;
    }
    setWidthPercent(sliderValue);

    const minZoom = 0.1; // 10% (super zoomed out)
    const maxZoom = 4.0; // 400% (super zoomed in)

    // Map slider 0–100 to minZoom–maxZoom
    const zoomFactor = minZoom + (sliderValue / 100) * (maxZoom - minZoom);
