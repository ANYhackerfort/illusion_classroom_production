...
export const VideoSocketProvider: React.FC<VideoSocketProviderProps> = ({
  ...
  const { socket, sendMessage, startMeeting, updateVideoState, getOneUpdate, connected } =
    useVideoSocket(safeRoomName(roomName));

  return (
    <VideoSocketContext.Provider
      value={{ socket, startMeeting, sendMessage, updateVideoState, getOneUpdate, connected }}
    >
      {children}
    </VideoSocketContext.Provider>
  );
