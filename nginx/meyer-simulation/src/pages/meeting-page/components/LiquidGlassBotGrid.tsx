...
import { getAllBotsFromServer } from "../../../components/botEditor/interfaces/bot_drop";
...
const LiquidGlassBotGrid: React.FC<Props> = ({ meetingName }) => {
        ...
        <div className="grid-inner">
          ...
          {bots.map((bot, i) => (
            <NameOverlayCardGlass
              key={i}
              image={`/media/bot_images/${bot.name}.jpg`}
              name={bot.name}
              ratio="16 / 9"
              videoSrc={`http://localhost:8081${bot.video_url ?? ""}`}
            />
          ))}
        </div>
      </div>
